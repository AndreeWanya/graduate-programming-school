...
class Car():
    ...
    def foo(self):						# Новое для задания 4.2
        return 'Car'
...
# Новое для задания 4.2
new_car = Car('white', 'sedan', 2000)
print(new_car.foo())
print(my_car.foo())
print(my_wife_car.foo())
cars = []
for i in range(500):
	new_car = random.choice([Daihatsu('белый', 'хэтчбек', 180, 'charade', 1999, 6, 4), Nissan('черный', 'хэтчбек', 144, 'leaf', 2015, 15, 30)])
	cars.append(new_car)

for car in cars:
	print(car.foo())	# Выводится переопределенный метод foo() для дочерних классов Daihatsu и Nissan, в зависимости от того, объект какого класса сейчас находится в переменной car
